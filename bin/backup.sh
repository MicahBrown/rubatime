...
if [[ -z "$S3_BACKUP_BUCKET_PATH" ]]; then
  echo "Missing S3_BACKUP_BUCKET_PATH variable which must be set the directory in s3 where you would like to store your database backups"
  exit 1
fi
...
${aws_command} s3 cp $FINAL_FILE_NAME s3://$S3_BACKUP_BUCKET_PATH/$APP/$DATABASE/$FINAL_FILE_NAME
